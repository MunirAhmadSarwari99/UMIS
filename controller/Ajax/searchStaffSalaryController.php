...
if (mysqli_num_rows($query) > 0){
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        $output .='<td>'. $pay .'</td>';
        $output .='<td class="setting text-right"><a href="../controller/deleteStaffSalaryController.php?id=' .$row["id"].'" class="btn btn-icon-toggle" data-toggle="tooltip" data-placement="top" data-original-title="Delete Row"><i class="fa fa-trash-o"></i></a></td>';
        $output .='</tr>';
    }
    echo $output;
}else{
